...
createApp({
    ...
    methods: {
        ...
        addMessage() {
            const currentTime = dt.now().setLocale('it').toLocaleString(dt.DATETIME_SHORT_WITH_SECONDS);
        ...
        deleteMessage(clickedIndex) {
            this.contacts[this.indexChat].messages.splice(clickedIndex, 1);
        },

        messageTime(time) {
            const luxonTime = dt.fromFormat(time, "dd/MM/yyyy, HH:mm:ss");
